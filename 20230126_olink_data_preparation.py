    ...
    for ind in givendata.index:
        ...
        if not dataattributes.loc[dataattributes.Sample.astype(
            ...
            if (currentattribute["Day"].item() <= 10):
                phase = pd.Series(["1-10d"], name="Phase")
            elif (currentattribute["Day"].item() >= 11 and currentattribute["Day"].item() < 31):
                phase = pd.Series(["11-30d"], name="Phase")
            elif (currentattribute["Day"].item() >= 31):
                phase = pd.Series([">30d"], name="Phase")
...
covum_attributes = pd.read_csv(
    "C:/umea_immunology/experiments/corona/olink_data/formatted_data/20211116_olink_cohort_orebro_formatted.csv")
covum_attributes = covum_attributes.fillna(0)

mpo_data = pd.read_csv(
    "C:/umea_immunology/experiments/corona/olink_data/formatted_data/20221118_olink_COVID_complete_MPODNA.csv")
...
complete_data.to_csv(
    "C:/umea_immunology/experiments/corona/olink_data/formatted_data/" + datetime.today().strftime("%Y%m%d") + "_Forsell_NPX_edit_complete.csv", index=False)
